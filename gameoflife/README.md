...
python3 life.py --pattern gun --preview                # print a pattern's shape once and exit, no simulation
```

Press Ctrl+C to stop early. `--preview` works with `--pattern <name>` or `--load <file>` (not the
default random board, since there's no fixed shape to preview) and ignores
`--width`/`--height` entirely, printing the pattern at its own natural size.
